...
public class BubbleSortMain extends JPanel {
	...
	private BubbleSortMain() {
		...
		// Stop the program when frame closes.	
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		// Set the size of the frame.
		frame.setSize(600, 300);
		// Set content pane to self for easy graphics.
		...
		frame.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent event) {
				// If the unsorted array exists and the sorting is not completed
				...
				// Otherwise, setup everything needed for the program to work
				else {
					// Sorting is not uncompleted
					completed = false;
	...
	private void render() {
		...
		for(int i = 0; i < unsortedArray.length; i++) {
			 ...
			 *  Otherwise, make the next index red to make it pop, 
			 *  the one it is being compared against blue.
			 *  All the others are made white.
			 */
			if (completed) 
				graphics.setColor(Color.getHSBColor((float) i / (unsortedArray.length - 1), 1f, 1f));
			else
				if (i == ((currentIndex != unsortedArray.length - 2) ? currentIndex + 1 : 0)) graphics.setColor(Color.RED);
				else if (i == ((currentIndex != unsortedArray.length - 2) ? currentIndex + 2 : 1)) graphics.setColor(Color.BLUE);
